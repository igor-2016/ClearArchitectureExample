...
namespace Collecting.Interfaces
{
    public enum CollectErrors
    {
        ...
        [ResponseError(Message = "Корзинка не найдена", HttpStatus = HttpStatusCode.BadRequest)]
        InvalidBasketId = 7017,

        /// <summary>
        /// Ошибочный номер корзины
        /// </summary>
        [ResponseError(Message = "Wrong basketGuid", HttpStatus = HttpStatusCode.BadRequest)]
        WrongBasketGuid = 7018,

        /// <summary>
        /// Невозможно начать отборку данного заказа
        /// </summary>
        [ResponseError(Message = "Cannot start collecting", HttpStatus = HttpStatusCode.BadRequest)]
        CannotStartCollect = 7019,
        ...
        [ResponseError(Message = "Внутренняя ошибка", HttpStatus = HttpStatusCode.BadRequest)]
        InternalError = 7021,

        [ResponseError(Message = "Признак весса не указан", HttpStatus = HttpStatusCode.BadRequest)]
        IsWeightedNotSet = 7022,

        [ResponseError(Message = "Нельзя найти ECom код доставки для Fozzy", HttpStatus = HttpStatusCode.BadRequest)]
        CannotMapDeliveryTypeFromFozzyToEcom = 7023,



        [ResponseError(Message = "Нельзя найти сотрудника по коду", HttpStatus = HttpStatusCode.NotFound)]
        NotFoundEmployeeByGlobalUserId = 7024,

        [ResponseError(Message = "Нельзя найти сотрудника по ИНН", HttpStatus = HttpStatusCode.NotFound)]
        NotFoundEmployeeByInn = 7025,


        [ResponseError(Message = "Ошибка поиска сотрудника по коду", HttpStatus = HttpStatusCode.InternalServerError)]
        PublicErrorGetEmployeeByGlobalUserId = 7026,

        [ResponseError(Message = "Ошибка поиска сотрудника по ИНН", HttpStatus = HttpStatusCode.InternalServerError)]
        PublicErrorGetEmployeeByInn = 7027,


        [ResponseError(Message = "Необработанная oшибка поиска сотрудника по коду", HttpStatus = HttpStatusCode.InternalServerError)]
        UnhandledErrorGetEmployeeByGlobalUserId = 7028,

        [ResponseError(Message = "Необработанная ошибка поиска сотрудника по ИНН", HttpStatus = HttpStatusCode.InternalServerError)]
        UnhandledErrorGetEmployeeByInn = 7029,

        [ResponseError(Message = "Ошибка формата данных при поиске сотрудника по коду", HttpStatus = HttpStatusCode.NotFound)]
        InvalidFormatEmployeeByGlobalUserId = 7030,

        [ResponseError(Message = "Ошибка формата данных при поиске сотрудника по ИНН", HttpStatus = HttpStatusCode.NotFound)]
        InvalidFormatEmployeeByInn = 7031,  
        
        [ResponseError(Message = "Не указан товар из каталога", HttpStatus = HttpStatusCode.NotFound)]
        NotFoundCatalogItem = 7032,

        [ResponseError(Message = "Надо использовать или товар из каталога или код в ECom.Core", HttpStatus = HttpStatusCode.NotFound)]
        InvalidUsageSearchOrderItem = 7033,

        /// <summary>
        /// Для интерфейса контроллера
        /// </summary>
        [ResponseError(Message = "Для проверки интерфейса ошибки", HttpStatus = HttpStatusCode.BadRequest)]
        BusinessExceptionTest = 7034,

        [ResponseError(Message = "Ожидается 922 статус с ТСД", HttpStatus = HttpStatusCode.BadRequest)]
        InvalidTsdState = 7035,

        [ResponseError(Message = "Ожидается 911 статус перед проверкой заказа в ТСД", HttpStatus = HttpStatusCode.BadRequest)]
        InvalidOrderStatusBeforeReadyToCheckState = 7036, 
        
        [ResponseError(Message = "Ошибка установки статуса 922 в ECom.Workflow", HttpStatus = HttpStatusCode.BadRequest)]
        ErrorSetReadyToCheckCollectingInWorkflow = 7037,
    }
}
